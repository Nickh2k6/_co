...
// --- 新增這段檢查 ---
  if (fp == NULL) {
      printf("Error: Could not open input file '%s' in pass1.\n", inFile);
...
// --- 新增這段檢查 ---
  if (fp == NULL) {
      printf("Error: Could not open input file '%s' in pass2.\n", inFile);
      return;
  }
  // -------------------
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
        printf("Usage: ./asm <filename_without_extension>\n");
        printf("Example: ./asm Add\n");
        return 1;
    }
  assemble(argv[1]);
}
